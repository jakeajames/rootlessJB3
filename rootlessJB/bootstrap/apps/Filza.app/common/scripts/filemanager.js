...
var setDimensions = function(){
    ...
    $('#filetree').height(newH-$('#search').height());
	$('#splitter, #leftpanel, #fileinfo, .vsplitbar').height(newH);

	var newW = $('#splitter').width() - 6 - $('#leftpanel').width();
    $('#fileinfo').width(newW);
};
...
var cleanString = function(str) {
 ...
 cleaned = cleaned.replace(/[_]+/g, "%20");
...
var createFileTree = function() {
	// Creates file tree.
 console.log('createFileTree: ' + fullexpandedFolder);
 $('#filetree').fileTree({
		root: fileRoot,
...
var renameItem = function(data) {
	...
	var getNewName = function(v, m){
		...
		if(rname != ''){
			...
			var givenName = encodeURIComponent(rname);
			...
			$.ajax({
				...
				success: function(result){
                    if(result['Code'] == 0){
                        ...
                        console.log('fullexpandedFolder: ' + newPath);
...
var addFolder = function(parent, name) {
	var newNode = '<li class="directory collapsed" style="background-image: url(\''+ data[key]['Preview'] + '\');background-size: 16px 16px; background-repeat: no-repeat;"><a data-path="' + parent + name + '/" href="#">' + name + '</a><ul class="jqueryFileTree" style="display: block;"></ul></li>';
...
var getFileInfo = function(file) {
	...
	$.getJSON(fileConnector + '?mode=getinfo&path=' + encodeURIComponent(file) + '&time=' + d.getMilliseconds(), function(data){
		if(data['Code'] == 0){
			...
			if(data['Properties']['Permissions'] && data['Properties']['Permissions'] != '') properties += '<dt>' + 'Permissions' + '</dt><dd>' + data['Properties']['Permissions'] + '</dd>';
...
var getFolderInfo = function(path) {
	...
	$.getJSON(url, function(data){
		...
		if(data){
			...
			var keys = data["___sorted_keys___"];
			var dict = data["___dict___"];
			...
			if($('#fileinfo').data('view') == 'grid'){
				...
				for(var i = 0; i < keys.length; i++){
					var key = keys[i];
					var props = dict[key]['Properties'];
					var cap_classes = "";
					for (cap in capabilities) {
						if (has_capability(dict[key], capabilities[cap])) {
					...
					config.options.showTitleAttr ? title = ' title="' + dict[key]['Path'] + '"' : title = '';
				
					result += '<li class="' + cap_classes + '"' + title + '"><div class="clip"><img src="' + dict[key]['Preview'] + '" width="' + scaledWidth + '" alt="' + dict[key]['Path'] + '" data-path="' + dict[key]['Path'] + '" /></div><p>' + dict[key]['Filename'] + '</p>';
					if(props['Permissions'] && props['Permissions'] != '') result += '<span class="meta dimensions">' + props['Permissions'] + '</span>';
				...
				result += '<thead><tr><th class="headerSortDown"><span>' + lg.name + '</span></th><th><span>' + 'Permissions' + '</span></th><th><span>' + lg.size + '</span></th><th><span>' + lg.modified + '</span></th></tr></thead>';
				result += '<tbody>';
				
				for(var i = 0; i < keys.length; i++){
					var key = keys[i];
					var path = dict[key]['Path'];
					var props = dict[key]['Properties'];
					var cap_classes = "";
					config.options.showTitleAttr ? title = ' title="' + dict[key]['Path'] + '"' : title = '';
					...
					result += '<td style="background-image: url('+ dict[key]['Preview'] + ');background-size: 16px 16px; background-repeat: no-repeat;" data-path="' + dict[key]['Path'] + '"' + title + '">' + dict[key]['Filename'] + '</td>';

					if(props['Permissions'] != ''){
						result += ('<td>' + props['Permissions'] + '</td>');
					} else {
						result += '<td></td>';
					}
...
var populateFileTree = function(path, callback) {
	var d = new Date(); // to prevent IE cache issues
 console.log("populateFileTree " + path);
	var url = fileConnector + '?path=' + encodeURIComponent(path) + '&mode=getfolderonly&showThumbs=' + config.options.showThumbs + '&time=' + d.getMilliseconds();
	if ($.urlParam('type')) url += '&type=' + $.urlParam('type');
	$.getJSON(url, function(data) {
		var result = '';
		// Is there any error or user is unauthorized?
		...
		if(data) {
			...
			for(var i = 0; i < keys.length; i++) {
				...
				if (dict[key]['File Type'] == 'dir') {
					result += "<li class=\"directory collapsed\" style=\"background-image: url('"+ dict[key]['Preview'] + "');background-size: 16px 16px; background-repeat: no-repeat;\"><a href=\"#\" class=\"" + cap_classes + "\" data-path=\"" + dict[key]['Path'] + "\">" + dict[key]['Filename'] + "</a></li>";
				} else {
					if(config.options.listFiles) {
					result += "<li class=\"file ext_" + dict[key]['File Type'].toLowerCase() + "\"><a href=\"#\" class=\"" + cap_classes + "\" data-path=\"" + dict[key]['Path'] + "\">" + dict[key]['Filename'] + "</a></li>";
					}
				}
			}
